...
**Error Conditions:**
- `422 Unprocessable Entity` jika payment sedang diproses atau sudah final
- `400 Bad Request` jika max retry sudah tercapai

---

### 3.6 Reconciliation

Menjalankan rekonsiliasi untuk payment dengan status tidak pasti (`PENDING_RECONCILIATION`).
...
| --- | --- |
| `Authorization` | `Bearer <operations_api_key>` |
| `Idempotency-Key` | `reconcile-order-10001` |
| `Content-Type` | `application/json` |
...
| Parameter | Type | Description |
| --- | --- | --- |
| `payment_id` | `string` | ID pembayaran yang akan direkonsiliasi |
...
```json
{
  "data": {
    "payment_id": "pay_01J8ZVX8B8",
    "previous_status": "PENDING_RECONCILIATION",
    "current_status": "SUCCESS",
    "provider_status": "COMPLETED",
    "resolution": "COMPLETED",
    "reconciled_at": "2026-09-13T12:00:00Z"
  }
}
```
...
Mencari pembayaran dengan filter.
...
| --- | --- | --- | --- |
| `merchant_reference` | `string` | Tidak | Filter berdasarkan merchant reference |
| `status` | `string` | Tidak | Filter berdasarkan status |
| `provider` | `string` | Tidak | Filter berdasarkan provider |
| `from_date` | `string (ISO 8601)` | Tidak | Filter created_at >= tanggal ini |
| `to_date` | `string (ISO 8601)` | Tidak | Filter created_at <= tanggal ini |
| `page` | `integer` | Tidak | Halaman (default: 1, min: 1) |
| `limit` | `integer` | Tidak | Jumlah per halaman (default: 20, max: 100) |
...
---
| `GET` | `/api/v1/payments/{payment_id}` | Mendapatkan detail pembayaran | Merchant API Key |
...
| `POST` | `/api/v1/payments/{payment_id}/retry` | Retry manual pembayaran | Operations Key |
| `POST` | `/api/v1/payments/{payment_id}/reconcile` | Rekonsiliasi status ke provider | Operations Key |
| `POST` | `/api/v1/webhooks/{provider}` | Menerima webhook dari provider | HMAC Signature |
